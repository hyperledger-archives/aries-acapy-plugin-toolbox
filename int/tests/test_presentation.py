...
@pytest.mark.asyncio
async def test_reject_presentation(
    backchannel: Client,
    issue_credential,
    issuer_holder_connection,
    wait_for_message,
    ...
    """Test the presentation rejection message and flows."""
    verifier, prover = issuer_holder_connection
    verifier = cast(ConnRecord, verifier)
    prover = cast(ConnRecord, prover)
    assert not isinstance(verifier.connection_id, Unset)
    ...
    assert presentation_request_received["matching_credentials"]
    msg_desc = "I reject your reality and substitute my own."
    await connection.send_async(
        {
    ...
    pres_sent = await wait_for_message(
        msg_type="did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/admin-holder/0.1/presentation-reject-sent"
    )
    assert pres_sent["~thread"]["thid"] == "3fa85f64-5717-4562-b3fc-2c963f66afa6"
    assert (
        pres_sent["presentation_exchange_id"]
        == presentation_request_received["presentation_exchange_id"]
    )
